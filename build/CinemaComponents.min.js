// Cinema Components v2.3. Copyright 2018 Los Alamos National Laboratory
...
var getAndParseCSV=function(path,callback,errorCallback){var request=new XMLHttpRequest();request.open("GET",path,true);request.onreadystatechange=function(){if(request.readyState===4){if(request.status===200){var data=parseCSV(request.responseText);if(callback)
callback(data);}
...
if(data[i].length!=testLength)
return"Each line must have an equal number of comma separated values (columns).";if(data[0].length!==dimensions.length+2)
return"All dimensions must be specified in the header of the file."
for(var i=2;i<data[0].length;i++){if(!dimensions.includes(data[0][i]))
...
CINEMA_COMPONENTS.Pcoord.prototype.setAxisOrder=function(order){var self=this;this.x.domain(order.filter(function(d){return self.dimensions.includes(d);}));self.dimensions.sort(function(a,b){return self.getXPosition(a)-self.getXPosition(b);});this.axes.attr('transform',function(d){return"translate("+self.getXPosition(d)+")";});this.redrawPaths();}
...
(this.parentRect.width)+' '+
(this.parentRect.height)).attr('preserveAspectRatio','none').attr('width','100%').attr('height','100%');this.selectedPaths=this.svg.append('g').classed('selectedPaths',true);this.highlightedPaths=this.svg.append('g').classed('highlightedPaths',true);this.overlayPaths=this.svg.append('g').classed('overlayPaths',true);this.redrawPaths();}
