...
#### Version 1.4 is not backwards compatible. Client programs will need changes to work with it

## Creating a new chart
To create a new chart, simply call the constructor like so:
```javascript
	var chart = new ParallelCoordinatesChart(parent, pathToCSV, filterRegex, callback);
```
#### Arguments
...
* **ParallelCoordinatesChart.dimensions**: An array of the dimensions used in the chart. (This will not include dimensions filtered out in the **filterRegex** argument of the constructor).
* **ParallelCoordinatesChart.allDimensions**: An array of all dimensions for hte data (including the ones filtered out)
...
* **ParallelCoordinatesChart.getSimiliar(data, threshold)**: Get all the results (as an array of indices) the are similiar to the given data. **threshold** is the maximum difference for results to be included. Difference is measured as the Manhattan distance where each dimension is normalized. i.e: The sum of the differences on each dimension (scaled from 0 to 1.0).
...
* **ParallelCoordinatesChart.isStringDimension(dimension)**: Returns a boolean value representing whether or not the given dimension is of type string (otherwise it is either of type float or integer).
...
/*Axis lines (for the NaN section of the axis)*/
.pCoordChart .axis path.NaNExtension, .pCoordChart .axis .NaNExtensionTick line {}
/*Axis text (for the NaN section of the axis)*/
.pCoordChart .axis .NaNExtensionTick text
```
...
- getIncompletePath() removed. Now getPath() works the way getIncompletePath() did
- Filters are now handled with a regex
